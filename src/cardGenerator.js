...
async function logoComposite() {
  ...
  const logo = await sharp(logoPath)
    ...
    .composite([{ input: Buffer.from(`<svg width="${size}" height="${size}"><circle cx="${size/2}" cy="${size/2}" r="${size/2}" fill="white"/></svg>`), blend: 'dest-in' }])
    .png().toBuffer();
  return [{ input: logo, left: 35, top: 18 }];
}
...
<svg width="${WIDTH}" height="${HEIGHT}" viewBox="0 0 ${WIDTH} ${HEIGHT}" xmlns="http://www.w3.org/2000/svg">
  <defs>
    ...
    <style>
      .topTitle{font-family:Impact,Arial Black,Arial,sans-serif;font-size:70px;font-style:italic;font-weight:900;fill:#050505;letter-spacing:1px}
      .white{font-family:Arial Black,Arial,sans-serif;font-weight:900;fill:white}
      .cat{font-family:Arial Black,Arial,sans-serif;font-size:32px;font-weight:900;fill:#050505}
      .marca{font-family:Arial Black,Arial,sans-serif;font-size:40px;font-weight:900;fill:#777;letter-spacing:1px}
      .produto{font-family:Impact,Arial Black,Arial,sans-serif;font-size:62px;font-weight:900;fill:#050505;letter-spacing:1px}
      .bullet{font-family:Arial Black,Arial,sans-serif;font-size:31px;font-weight:900;fill:#080808}
      .price{font-family:Impact,Arial Black,Arial,sans-serif;font-size:72px;font-weight:900;fill:#ffd000;letter-spacing:1px}
      .old{font-family:Arial Black,Arial,sans-serif;font-size:34px;font-weight:900;fill:#aaa}
      .small{font-family:Arial Black,Arial,sans-serif;font-size:31px;font-weight:900;fill:white}
      .red{fill:#f00010}.yellow{fill:#ffcc00}.black{fill:#050505}
    </style>
  </defs>

  <rect width="1080" height="1350" fill="url(#bg)"/>
  <path d="M200 0 H835 L780 210 H0 V0 Z" fill="#ffcc00"/>
  <text x="300" y="90" class="topTitle">PROMO TCHÊ</text>
  <rect x="300" y="122" width="460" height="48" rx="24" fill="#050505"/>
  <text x="355" y="155" class="white" font-size="26">OFERTAS TODOS OS DIAS!</text>
  ...
  <text x="855" y="318" class="white" font-size="46">${desconto || 0}%</text>
  <text x="865" y="368" class="white" font-size="42" fill="#ffcc00">OFF</text>
  ...
  <line x1="200" y1="952" x2="410" y2="935" stroke="#f00010" stroke-width="5"/>
  ...
  <rect x="675" y="1006" width="305" height="94" rx="12" fill="#f00010" stroke="white" stroke-width="4" stroke-dasharray="10 8"/>
  <text x="690" y="1065" class="white" font-size="33" fill="#ffcc00">${escapeXml(cupom || 'CONFIRA')}</text>

  <rect x="60" y="1192" width="595" height="58" rx="8" fill="#050505"/>
  <text x="85" y="1229" class="white" font-size="25">🔗 LINK NA BIO • OFERTAS TODOS OS DIAS!</text>
  <rect x="675" y="1192" width="350" height="58" rx="9" fill="#ffcc00"/>
  <text x="720" y="1220" class="cat" font-size="24">CORRE QUE É</text>
  <text x="702" y="1245" class="cat" font-size="24">POR TEMPO LIMITADO!</text>
</svg>`;
}
...
async function gerarCard(dados = {}, baseUrl = '') {
  ...
  if (productBuffer) {
    const product = await sharp(productBuffer)
      .resize(380, 440, { fit: 'contain', background: { r: 255, g: 255, b: 255, alpha: 0 } })
      .png().toBuffer();
    composites.push({ input: product, left: 610, top: 365 });
  }
