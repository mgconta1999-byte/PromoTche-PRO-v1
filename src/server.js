...
app.use('/cards', express.static(path.join(__dirname, '..', 'generated')));

app.get('/', (req, res) => {
  res.json({ ok: true, app: 'PromoTche PRO v2', rota: 'POST /api/gerar-card' });
});
...
app.listen(PORT, () => {
  console.log(`PromoTche PRO v2 rodando em http://localhost:${PORT}`);
});
